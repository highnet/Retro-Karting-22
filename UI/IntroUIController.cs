...
public class IntroUIController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if(SaveSystem.LoadRecords() == null || PlayerPrefs.GetInt("ResettedRecordsSince160420220025", 0) == 0) // check if there are no records on file or the records havent been force resetted since a specific timestamp with the force reset tag
        {
            SaveSystem.SaveRecords(SaveSystem.GenerateDefaultRecords()); // generate default records and save them on file
            PlayerPrefs.SetInt("ResettedRecordsSince160420220025", 1); // set the force reset tag on player prefs
        ...
        if (SaveSystem.LoadCollectibles() == null || PlayerPrefs.GetInt("ResettedCollectiblesSince160420220025", 0) == 0)
        {
            SaveSystem.SaveCollectibles(SaveSystem.GenerateDefaultCollectibles());
            PlayerPrefs.SetInt("ResettedCollectiblesSince160420220025", 1);
                PlayerPrefs.Save();
        }

        if (SaveSystem.LoadGhostRider()  == null || PlayerPrefs.GetInt("ResettedGhostRiderSince160420220025", 0) == 0)
        {
            SaveSystem.SaveGhostRider(SaveSystem.GenerateDefaultGhostRider());
            PlayerPrefs.SetInt("ResettedGhostRiderSince160420220025", 1);
            PlayerPrefs.Save();
        }
